...
with torch.no_grad():
    ...
    for search_step in trange(1000):
        ...
        losses = ((deeponet_y_hats - ys)**2).mean(dim=(1,2))

        # get the hardest example
        max_index = torch.argmax(losses)
        if losses[max_index] > deeponet_loss:
