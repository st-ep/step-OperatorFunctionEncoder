...
##############   Evaluate    ###################
with torch.no_grad():
    # Add denormalize function definition
    def denormalize(y):
        mean, std = (2.8366714104777202e-05, 4.263603113940917e-05)
        return y * std + mean
        
    # Add these variables to track total relative losses
    total_relative_b2b_loss = 0
    total_relative_deeponet_loss = 0
    n_samples = 50
    start_index = 100

    print("\nCalculating average relative MAE across last 50 samples...")
    for i in trange(start_index, start_index + n_samples):
        info_i = {'function_indicies': torch.tensor([i], device=device)}
        
        # Sample inputs and get ground truth outputs
        xs_i = testing_combined_dataset.tgt_dataset.sample_inputs(info_i, None)
        ys_i = testing_combined_dataset.tgt_dataset.compute_outputs(info_i, xs_i)
        
        # Get corresponding source inputs and outputs
        example_xs_i = testing_combined_dataset.src_dataset.sample_inputs(info_i, None)
        example_ys_i = testing_combined_dataset.src_dataset.compute_outputs(info_i, example_xs_i)
        
        # Get b2b predictions
        rep_i, _ = b2b_model["src"].compute_representation(example_xs_i, example_ys_i, method=args.train_method)
        rep_i = b2b_model["A"](rep_i)
        b2b_y_i = b2b_model["tgt"].predict(xs_i, rep_i)
        
        # Get DeepONet predictions
        deeponet_y_i = deeponet_model.forward(example_xs_i, example_ys_i, xs_i)
        
        # Denormalize predictions and ground truth
        ys_i_denorm = denormalize(ys_i)
        b2b_y_i_denorm = denormalize(b2b_y_i)
        deeponet_y_i_denorm = denormalize(deeponet_y_i)
        
        # Get max displacement for this specific sample
        max_displacement = ys_i_denorm.abs().max()
        
        # Accumulate relative losses (normalize by max_displacement for each sample)
        sample_b2b_loss = torch.mean(torch.abs(b2b_y_i_denorm - ys_i_denorm))
        sample_deeponet_loss = torch.mean(torch.abs(deeponet_y_i_denorm - ys_i_denorm))
        total_relative_b2b_loss += sample_b2b_loss / max_displacement
        total_relative_deeponet_loss += sample_deeponet_loss / max_displacement
    
    # Calculate average relative losses
    avg_relative_b2b_loss = total_relative_b2b_loss / n_samples
    avg_relative_deeponet_loss = total_relative_deeponet_loss / n_samples
    
    # Convert to percentages (multiply by 100)
    print(f"\nAverage Relative MAE across last {n_samples} samples:")
    print(f"Average B2B Relative Loss: {(avg_relative_b2b_loss * 100):.4f}%")
    print(f"Average DeepONet Relative Loss: {(avg_relative_deeponet_loss * 100):.4f}%")
    print(f"Relative Difference: {(abs(avg_relative_b2b_loss - avg_relative_deeponet_loss) * 100):.4f}%")
    print(f"Ratio: {avg_relative_deeponet_loss / avg_relative_b2b_loss:.4f}")
    ...
    b2b_sum = 3000*b2b_y_offset1_denorm + 2000*b2b_y_offset2_denorm
    ...
    deeponet_sum = 3000*deeponet_y_offset1_denorm + 2000*deeponet_y_offset2_denorm
    ...
    b2b_loss = torch.mean((b2b_y_hardest_denorm - ys_hardest_denorm)**2)
    deeponet_loss = torch.mean((deeponet_y_hardest_denorm - ys_hardest_denorm)**2)
    print(f"\nAverage Losses (MSE):")
    print(f"B2B Loss: {b2b_loss:.6e}")
    print(f"DeepONet Loss: {deeponet_loss:.6e}")
    print(f"Difference: {abs(b2b_loss - deeponet_loss):.6e}")
    print(f"Ratio: {deeponet_loss / b2b_loss:.6e}")
